...
Item {
  ...
  readonly property int updateCount: pluginApi?.mainInstance?.updateCount ?? 0
  ...
  readonly property real contentWidth: isVertical ? capsuleHeight : grid.implicitWidth + Style.marginS * 2
  readonly property real contentHeight: isVertical ? grid.implicitHeight + Style.marginS * 2 : capsuleHeight
  ...
  MouseArea {
    ...
    onPressed: mouse => {
      TooltipService.hide();
      if (mouse.button === Qt.LeftButton && root.updateCount > 0)
  ...
  function showTooltip() {
    const key = root.updateCount === 0 ? "tooltip.noUpdatesAvailable" : "tooltip.updatesAvailable";
    TooltipService.show(root, pluginApi?.tr(key), BarService.getTooltipDirection(root.screenName));
  }
}
